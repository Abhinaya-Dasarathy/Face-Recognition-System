...
int main(int argc, char *argv[]) {
    ...
    cout<<"Initiating Process "<<my_rank<<endl;
    ...
    cout<<"CSV file Read by Process "<<my_rank<<endl;
    ...
    cout<<"Process "<<my_rank<<" is buiding a model."<<endl;
    Ptr<FaceRecognizer> model = createEigenFaceRecognizer();
    model->train(images, labels);

    predictedLabel = model->predict(testSample);
    cout<<"predicted label of Process "<<my_rank<<" is "<<predictedLabel<<endl;
    ...
    cout<<"Final reducion in Process "<<my_rank<<endl;
    ...
    //display results
    if(my_rank == 0) {
      cout<<"display_results from Process 0"<<endl;
	...
	cout<<"Writing the output to RecognizedFace.pgm"<<endl;
	const char *file_name = "./RecognizedFace.pgm";
