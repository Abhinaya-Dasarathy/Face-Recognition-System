...
void displayResults(IplImage * image, std::vector<CvRect> faces, char * windowTitle ,const char * outfile)
{
	...
	//const char* outputfileimage ="/data/users/adasarat/FaceDetectionGPU/FaceDetectionGPU/output.jpg";
	int p[3];
	p[0]=1;
	p[1] =10;
	p[2] =0;
	cvSaveImage(outfile, image, p);
	return;
...
int main( int argc, const char** argv )
{
	...
	std::vector<CvRect> gpuFaces_v1 = runGPUHaarDetection(scale, minNeighbors, V1);
	 printf("Size:%d",gpuFaces_v1.size());
	...
	// Show results
	const char *file1 ="./output_gpu.jpg";
	displayResults(gpuImage_v1, gpuFaces_v1, "GPU Results v1",file1);

const char *file2 ="./output_opencv.jpg";
	displayResults(opencvImage, opencvFaces, "OpenCV Results",file2);
